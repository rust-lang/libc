...
mod macro_checks {
    ...
    // Types configured out entirely. `cfg(any())` is false everywhere, so these check that the
    // generated impls carry the same `cfg` as the type. Without it they fail to compile with
    // "cannot find type".
    s_with_default! {
        #[cfg(any())]
        pub struct S5 {
