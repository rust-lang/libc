    ...
    if #[cfg(feature = "extra_traits")] {
        impl PartialEq for __c_anonymous_ifru_map {
            fn eq(&self, other: &__c_anonymous_ifru_map) -> bool {
                self.mem_start == other.mem_start
                    && self.mem_end == other.mem_end
        ...
        impl hash::Hash for __c_anonymous_ifru_map {
            fn hash<H: hash::Hasher>(&self, state: &mut H) {
                self.mem_start.hash(state);
                self.mem_end.hash(state);
...
pub const SEM_FAILED: *mut crate::sem_t = 0 as *mut crate::sem_t;
